...
function createSortLink($column, $text, $currentSortBy, $currentSortOrder, $currentSearch) {
    ...
    // Add an arrow indicator for the currently active column
    $indicator = '';
...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex space-x-4">
        <a href="index.php?page=kendaraan&action=recycleBin" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-300">
            Recycle Bin
        </a>
        <a href="index.php?page=kendaraan&action=create" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-300">
            Tambah Kendaraan
        </a>
    </div>
</div>
...
<div class="mb-6 bg-white p-4 rounded-lg shadow-md">
    <form action="index.php" method="GET" class="flex items-center space-x-4">
        ...
        <a href="index.php?page=kendaraan" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-300">
            Reset
        </a>
    </form>
</div>
    ...
    <table class="min-w-full leading-normal">
        <thead>
            <tr class="bg-gray-800 text-white">
                <th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold uppercase tracking-wider">
                    <?= createSortLink('id_kendaraan', 'ID', $sortBy, $sortOrder, $search) ?>
                </th>
                <th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold uppercase tracking-wider">
        ...
        <tbody>
            ...
            <tr class="hover:bg-gray-100">
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['id_kendaraan']) ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['jenis']) ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['merk']) ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['no_plat']) ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm">
                    <span class="px-2 py-1 font-semibold leading-tight rounded-full <?= $row['status'] == 'tersedia' ? 'bg-green-200 text-green-900' : 'bg-red-200 text-red-900' ?>">
                        <?= htmlspecialchars(ucfirst($row['status'])) ?>
                    </span>
                </td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm">
                    <a href="index.php?page=kendaraan&action=edit&id=<?= $row['id_kendaraan'] ?>" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded text-xs">Edit</a>
                    <a href="index.php?page=kendaraan&action=delete&id=<?= $row['id_kendaraan'] ?>" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded text-xs" onclick="return confirm('Yakin?')">Hapus</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
...
<div class="mt-6 flex justify-center">
    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
        <?php if($totalPages > 1): ?>
            <?php $paginationUrl = "index.php?page=kendaraan&q=" . urlencode($search) . "&sort_by=$sortBy&sort_order=$sortOrder"; ?>

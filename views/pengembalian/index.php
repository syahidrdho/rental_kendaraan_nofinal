...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="index.php?page=pengembalian&action=create" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-300">
...
<div class="mb-6 bg-white p-4 rounded-lg shadow-md">
    <form action="index.php" method="GET" class="flex items-center space-x-4">
        ...
        <a href="index.php?page=pengembalian" class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-2 px-4 rounded-lg shadow-md transition-colors duration-300">
            Reset
        </a>
    </form>
</div>
    ...
    <table class="min-w-full leading-normal">
        <thead>
            <tr class="bg-gray-800 text-white">
                <th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold uppercase tracking-wider"><?= createSortLink('id_pengembalian', 'ID Kembali', $sortBy, $sortOrder, $search) ?></th>
                ...
                <th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold uppercase tracking-wider"><?= createSortLink('tgl_dikembalikan', 'Tgl Dikembalikan', $sortBy, $sortOrder, $search) ?></th>
                <th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold uppercase tracking-wider"><?= createSortLink('denda', 'Denda', $sortBy, $sortOrder, $search) ?></th>
        ...
        <tbody>
            ...
            <tr class="hover:bg-gray-100">
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['id_pengembalian']) ?></td>
                ...
                <td class="px-5 py-4 border-b border-gray-200 text-sm"><?= htmlspecialchars($row['tgl_dikembalikan']) ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm">Rp <?= number_format($row['denda'], 0, ',', '.') ?></td>
                <td class="px-5 py-4 border-b border-gray-200 text-sm">
                    <a href="index.php?page=pengembalian&action=edit&id=<?= $row['id_pengembalian'] ?>" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded text-xs">Edit</a>
                    <a href="index.php?page=pengembalian&action=delete&id=<?= $row['id_pengembalian'] ?>" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-3 rounded text-xs" onclick="return confirm('Yakin?')">Hapus</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
...
<div class="mt-6 flex justify-center">
    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
        <?php if($totalPages > 1): ?>
            <?php $paginationUrl = "index.php?page=pengembalian&q=" . urlencode($search) . "&sort_by=$sortBy&sort_order=$sortOrder"; ?>
